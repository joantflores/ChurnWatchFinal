...
def evaluar_dataset(nombre_dataset: str, X: pd.DataFrame, y: pd.Series) -> pd.DataFrame:
...
def benchmark_telco() -> pd.DataFrame:
    ...
    if not telco_path.exists():
        raise FileNotFoundError("No se encontro TelcoCustomerChurn.csv en la raiz del proyecto.")

    X_telco, y_telco = preparar_telco(telco_path)
    telco_res = evaluar_dataset("Telco", X_telco, y_telco)
    telco_res.to_csv(OUTPUT_DIR / "telco_metrics.csv", index=False)
    ...
    for i, (_, row) in enumerate(telco_res.iterrows()):
        ...
        bars = ax.bar(x + i * width, vals, width, label=row["Modelo"], color=colores[i], alpha=0.9)
        for b, v in zip(bars, vals):
            ax.text(b.get_x() + b.get_width() / 2, b.get_height() + 0.004, f"{v:.3f}", ha="center", va="bottom", fontsize=8)
    ax.set_xticks(x + width)
    ax.set_xticklabels(metricas)
    ax.set_ylim(0, 1.12)
    ax.set_ylabel("Score")
    ax.set_title("Benchmark Telco - Validacion cruzada (k=5)")
    ax.legend(loc="upper right", fontsize=9)
    ax.spines[["top", "right"]].set_visible(False)
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "telco_model_comparison.png", dpi=150, bbox_inches="tight")
    plt.close()
    return telco_res
...
def shap_shopping_best_model() -> None:
    ...
    if not model_artifact.exists():
        raise FileNotFoundError("No se encontro best_model.pkl. Ejecuta primero: python model_training.py")
    if not shopping_path.exists():
        raise FileNotFoundError("No se encontro shopping_trends.csv.")
    ...
    df = construir_churn(df_raw, verbose=False)
    X, y, local_scaler = preparar_features(df)

    if scaler is not None:
        X, y, _ = preparar_features(df, scaler=scaler, fit_scaler=False)
    else:
        scaler = local_scaler
    ...
    print(f"[OK] Mejor modelo en shopping listo para SHAP. AUC train={auc:.4f}")
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "shap_waterfall_cliente_1.png", dpi=150, bbox_inches="tight")
    plt.close()

    print(f"[OK] Graficas SHAP guardadas en {OUTPUT_DIR}")
...
def main() -> None:
    ...
    print(telco_df.round(4).to_string(index=False))
