...
# cargar dataset y normalizar columnas
...
# preparar features numéricas y categóricas
def preprocess(df: pd.DataFrame):
    df = df.copy()

    if "churn" not in df.columns:
        raise ValueError(
            "El dataset no tiene columna 'churn'. "
            "Ejecuta build_churn.py primero."
        )
    ...
    if y.nunique() < 2:
        ...
        raise ValueError(
            ...
            f"Distribucion encontrada: {counts}. "
            "Ejecuta build_churn.py para regenerarla."
        )
    ...
    cat_cols = [
        ...
        if c not in set(num_cols) | drop_cols
    ]
...
def fit_and_eval(
    ...
    # fallback si train tiene una sola clase
    if y_train.nunique() < 2:
        print(
            "[WARN] Solo una clase presente "
            "en entrenamiento. Usando DummyClassifier."
        )
        ...
        results = {
            "ConstantBaseline": {
                "roc_auc": (
                    None
                    if probs is None
                    else float(roc_auc_score(y_test, probs))
                ),
                "precision": float(
                    precision_score(
                        y_test,
        ...
        print(
            "Fallback guardado como ConstantBaseline "
            f"en {OUT}"
        )

        return

    # crear pipeline sklearn
    ...
    from churn_engine import (
        BAJA_FRECUENCIA_CANON,
        CHURN_UMBRAL_ENTRENAMIENTO,
    )

    # payload completo del modelo
    payload = {
        "modelo": best_model,
        "scaler": None,
        ...
        "churn_config": {
            "umbral_previous_purchases":
                CHURN_UMBRAL_ENTRENAMIENTO,

            "baja_frecuencia":
                sorted(BAJA_FRECUENCIA_CANON),
        },
    }
    ...
    # guardar tabla CSV comparativa
    (
        pd.DataFrame
        .from_dict(results, orient="index")
        .drop(columns=["confusion_matrix"])
        .to_csv(OUT / "model_comparison.csv")
    )
...
def main():
    ...
    if not SHOP.exists():
        print(
            "Ejecuta build_churn.py primero "
            "para generar shopping_trends_churn.csv"
        )
